...
try {
    ...
    $stmt = $db->query("
        SELECT pa.*, o.price as offer_price, o.title as offer_title, o.id as offer_id, u.username
    ...
    foreach ($alerts as $alert) {
        ...
        // Check if price dropped below target and not already notified
        if ($currentPrice <= $targetPrice && $alert['is_notified'] == 0) {
            // Mark as notified
            $updateStmt = $db->prepare("
                UPDATE price_alerts 
                SET is_notified = 1, current_price = ? 
                WHERE id = ?
            ");
